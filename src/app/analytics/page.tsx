...
export default async function AnalyticsPage({
  ...
  const weekdayData = getWeekdayPerformance(insightTrades);
  ...
  return (
    <main className="px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-4">
        ...
        <section className="overflow-hidden rounded-[28px] border border-white/8 bg-[#2b2d31] shadow-[0_24px_60px_rgba(0,0,0,0.3)]">
          ...
          <div className="flex overflow-x-auto divide-x divide-white/8 border-t border-white/8">
            {[
              ...
              { label: "Win Rate", value: closedTrades.length ? formatPercent(winRate) : "--", cls: winRateColor(winRate) },
              {
                label: "Profit Factor",
                value: profitFactor.grossLoss > 0 ? profitFactor.profitFactor.toFixed(2) : profitFactor.grossProfit > 0 ? "∞" : "--",
                cls: profitFactor.profitFactor >= 1.5 ? "text-emerald-400" : profitFactor.profitFactor >= 1 ? "text-[#b5bac1]" : "text-rose-400",
              },
              {
                label: "W / L Ratio",
              ...
              { label: "Avg Hold", value: holdingTime.avgHoldMinutes > 0 ? formatHoldTime(holdingTime.avgHoldMinutes) : "--", cls: "text-white" },
              {
                label: "Max DD",
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-white/8 bg-[#1e1f22]/60 px-5 py-2">
            <span className="text-[11px] text-[#949ba4]">Reviewed <span className="text-white">{reviewSummary.reviewedTrades}</span></span>
            <span className="select-none text-white/20">·</span>
            <span className="text-[11px] text-[#949ba4]">Grade A/B <span className="text-white">{reviewSummary.gradeCounts.A + reviewSummary.gradeCounts.B}</span></span>
            <span className="select-none text-white/20">·</span>
            <span className="text-[11px] text-[#949ba4]">Followed Plan <span className="text-white">{formatPercent(reviewSummary.followedPlanRate)}</span></span>
            {reviewSummary.avgConfidence > 0 && (
              <><span className="select-none text-white/20">·</span><span className="text-[11px] text-[#949ba4]">Conf <span className="text-white">{reviewSummary.avgConfidence.toFixed(1)}/5</span></span></>
            )}
            {profitFactor.grossProfit > 0 && (
              <><span className="select-none text-white/20">·</span><span className="text-[11px] text-[#949ba4]">Gross <span className="text-emerald-400">{formatCurrency(profitFactor.grossProfit)}</span> / <span className="text-rose-400">{formatCurrency(-profitFactor.grossLoss)}</span></span></>
            )}
            {(drawdown.maxWinStreak > 0 || drawdown.maxLossStreak > 0) && (
              <><span className="select-none text-white/20">·</span><span className="text-[11px] text-[#949ba4]">Streak <span className="text-emerald-400">{drawdown.maxWinStreak}W</span>{" / "}<span className="text-rose-400">{drawdown.maxLossStreak}L</span></span></>
            )}
          </div>
        </section>
            ...
            <div className="mt-2 flex items-center justify-between rounded-[18px] bg-[#1e1f22] px-4 py-3">
              <div>
                ...
                <p className="mt-1.5 text-sm font-semibold text-white">
                  {drawdown.peak > 0 ? formatCurrency(drawdown.peak) : "--"}
                </p>
              </div>
                ...
                <table className="min-w-full divide-y divide-white/8">
                  <thead className="sticky top-0 z-10 bg-[#1e1f22] text-left text-[10px] uppercase tracking-[0.16em] text-[#949ba4]">
                    <tr>
                      ...
                      <th className="px-3 py-2"><Link href={sortHref(allParams, "setupSort", "setupSortDir", "totalTrades", setupSortKey, setupSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white"># <SortIndicator active={setupSortKey === "totalTrades"} dir={setupSortDir} /></Link></th>
                      <th className="px-3 py-2"><Link href={sortHref(allParams, "setupSort", "setupSortDir", "winRate", setupSortKey, setupSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white">Win% <SortIndicator active={setupSortKey === "winRate"} dir={setupSortDir} /></Link></th>
                      <th className="px-3 py-2"><Link href={sortHref(allParams, "setupSort", "setupSortDir", "totalPnl", setupSortKey, setupSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white">P&amp;L <SortIndicator active={setupSortKey === "totalPnl"} dir={setupSortDir} /></Link></th>
                      <th className="w-[72px] px-3 py-2"></th>
                      <th className="px-3 py-2"><Link href={sortHref(allParams, "setupSort", "setupSortDir", "avgPnl", setupSortKey, setupSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white">Avg <SortIndicator active={setupSortKey === "avgPnl"} dir={setupSortDir} /></Link></th>
                      <th className="px-3 py-2"><Link href={sortHref(allParams, "setupSort", "setupSortDir", "returnPercent", setupSortKey, setupSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white">Ret% <SortIndicator active={setupSortKey === "returnPercent"} dir={setupSortDir} /></Link></th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/6 text-[12px]">
                    {setupPerformance.map((setup) => (
                      <tr key={setup.setup} className="transition hover:bg-white/[0.02]">
                        <td className="px-3 py-2"><Link href={`/journal?setup=${encodeURIComponent(setup.setup)}&range=${selectedRange}`} className="font-medium text-white transition hover:text-[#5865f2]">{setup.setup}</Link></td>
              ...
              <table className="min-w-full divide-y divide-white/8">
                <thead className="sticky top-0 z-10 bg-[#1e1f22] text-left text-[10px] uppercase tracking-[0.16em] text-[#949ba4]">
                  <tr>
                    ...
                    <th className="px-3 py-2"><Link href={sortHref(allParams, "symbolSort", "symbolSortDir", "totalTrades", symbolSortKey, symbolSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white"># <SortIndicator active={symbolSortKey === "totalTrades"} dir={symbolSortDir} /></Link></th>
                    <th className="px-3 py-2">Cl</th>
                    <th className="px-3 py-2"><Link href={sortHref(allParams, "symbolSort", "symbolSortDir", "winRate", symbolSortKey, symbolSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white">Win% <SortIndicator active={symbolSortKey === "winRate"} dir={symbolSortDir} /></Link></th>
                    <th className="px-3 py-2"><Link href={sortHref(allParams, "symbolSort", "symbolSortDir", "totalPnl", symbolSortKey, symbolSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white">P&amp;L <SortIndicator active={symbolSortKey === "totalPnl"} dir={symbolSortDir} /></Link></th>
                    <th className="w-[72px] px-3 py-2"></th>
                    <th className="px-3 py-2"><Link href={sortHref(allParams, "symbolSort", "symbolSortDir", "avgPnl", symbolSortKey, symbolSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white">Avg <SortIndicator active={symbolSortKey === "avgPnl"} dir={symbolSortDir} /></Link></th>
                    <th className="px-3 py-2"><Link href={sortHref(allParams, "symbolSort", "symbolSortDir", "returnPercent", symbolSortKey, symbolSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white">Ret% <SortIndicator active={symbolSortKey === "returnPercent"} dir={symbolSortDir} /></Link></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/6 text-[12px]">
                  {symbolPerformance.map((sym) => (
                    <tr key={sym.symbol} className="transition hover:bg-white/[0.02]">
                      <td className="px-3 py-2"><Link href={`/journal?symbol=${encodeURIComponent(sym.symbol)}&range=${selectedRange}`} className="font-medium text-white transition hover:text-[#5865f2]">{sym.symbol}</Link></td>
                  ...
                  {symbolPerformance.length === 0 && (
                    <tr><td colSpan={8} className="px-3 py-4 text-[12px] text-[#b5bac1]">No trades in this range yet.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
                  ...
                  <table className="min-w-full divide-y divide-white/8">
                    <thead className="sticky top-0 z-10 bg-[#1e1f22] text-left text-[10px] uppercase tracking-[0.16em] text-[#949ba4]">
                      <tr>
                        ...
                        <th className="px-3 py-2"><Link href={sortHref(allParams, "tagSort", "tagSortDir", "totalTrades", tagSortKey, tagSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white"># <SortIndicator active={tagSortKey === "totalTrades"} dir={tagSortDir} /></Link></th>
                        <th className="px-3 py-2">Cl</th>
                        <th className="px-3 py-2"><Link href={sortHref(allParams, "tagSort", "tagSortDir", "winRate", tagSortKey, tagSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white">Win% <SortIndicator active={tagSortKey === "winRate"} dir={tagSortDir} /></Link></th>
                        <th className="px-3 py-2"><Link href={sortHref(allParams, "tagSort", "tagSortDir", "totalPnl", tagSortKey, tagSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white">P&amp;L <SortIndicator active={tagSortKey === "totalPnl"} dir={tagSortDir} /></Link></th>
                        <th className="w-[72px] px-3 py-2"></th>
                        <th className="px-3 py-2"><Link href={sortHref(allParams, "tagSort", "tagSortDir", "avgPnl", tagSortKey, tagSortDir)} scroll={false} className="flex items-center gap-1 transition hover:text-white">Avg <SortIndicator active={tagSortKey === "avgPnl"} dir={tagSortDir} /></Link></th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/6 text-[12px]">
                      {tagPerformance.map((row) => (
                        <tr key={row.tag} className="transition hover:bg-white/[0.02]">
                          <td className="px-3 py-2"><Link href={`/journal?tag=${encodeURIComponent(row.tag)}&range=${selectedRange}`} className="font-medium text-white transition hover:text-[#5865f2]">{row.tag}</Link></td>
        ...
        {/* ── Row D: Weekday Performance + Holding Time ── */}
